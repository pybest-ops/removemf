...
import { PlatformExamples, tiktokExamples } from '@/components/PlatformExamples';
...
// metadata 承接 TikTok 长尾词搜索意图，声明页面规范地址。
export function generateMetadata(): Metadata {
  const locale = getRequestLocale();
  const dictionary = getDictionary(locale);

  return createI18nMetadata(locale, '/tiktok-remove-matcha-filter', dictionary.metadata.tiktok);
}

// TiktokRemoveMatchaFilterPage 面向 TikTok 用户，复用上传组件并补充平台专属内容。
export default function TiktokRemoveMatchaFilterPage() {
  const locale = getRequestLocale();
  const dictionary = getDictionary(locale).tiktok;
    ...
    <main className="relative isolate min-h-screen overflow-hidden px-6 py-8">
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <p className="rounded-full border border-white/80 bg-white/60 px-4 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-matcha-700 shadow-sm backdrop-blur">TikTok matcha filter remover</p>
        </div>
        ...
        <div>
          ...
          <section className="relative grid gap-6 rounded-[2.5rem] border border-white/70 bg-white/65 p-6 shadow-[0_30px_100px_rgba(31,82,44,0.16)] backdrop-blur-xl md:p-8 lg:grid-cols-[0.95fr_1.05fr] lg:p-10">
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
              <InfoCard title="Step 1 · Screenshot or save" text="Take a screenshot of the TikTok video or save the photo with the matcha-style color cast." />
        ...
        {/* Before/After 示例展示 */}
        <PlatformExamples platform="tiktok" examples={tiktokExamples} />
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <Link className="rounded-3xl border border-white/80 bg-white/70 p-5 shadow-sm backdrop-blur transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(31,82,44,0.12)]" href={localizePath('/youtube-remove-matcha-filter', locale)}>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-matcha-700">Also available</p>
            <p className="mt-2 text-lg font-semibold text-slate-950">YouTube Remove Matcha Filter →</p>
          </Link>
